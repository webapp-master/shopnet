...
export const register = (name, email, password, wallet_balance) => async (dispatch) => {
    try {
      dispatch({
      ...
      const data = {
        ...
        email,
        password,
        wallet_balance, // Include the wallet_balance field
      };
  
      const { data: userData } = await axios.post('/api/users/register/', data, config);
